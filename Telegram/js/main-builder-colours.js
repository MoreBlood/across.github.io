...
$(document).ready(function () {
  ...
  $("#process").click(function (argument) {
    ...
    for(var key in json_obj){
      if (json_obj[key][0] == "#" && json_obj[key].length < 8 && $("#cb1").prop( "checked" ) == true)$("#output").append('<div ' + 'style="background:' + json_obj[key] + '" class="exported_colour"><span style="background: white">' + key +  ": <input class='hex' type=text value='"  + json_obj[key] + "'></div>");
    ...
    for(var key in json_obj){
      if (json_obj[key][0] == "#" && json_obj[key].length > 7 && $("#cb3").prop( "checked" ) == true)$("#output").append('<div ' + 'style="background:' + hexToRgbA(json_obj[key].substring(0,7)) +  parseInt(json_obj[key].substring(7,9), 16) / 256 + ')" class="exported_colour"><span style="background: white">' + key +  ": <input class='hex' type=text value='"  + json_obj[key] + "'></div>");
      //else $("#output").append('<div ' + 'style="background:' + json_obj[json_obj[key]] +  '" class="exported_colour"><span style="background: white">' + key + " copied <b>" + json_obj[key] + '</b></span></div>')
    }
